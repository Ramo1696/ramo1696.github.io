...
function updateAutoRate() {
  if (autoRateCheckbox.checked) {
    ...
    console.log(`Auto rate enabled. Fetching rate for ${crypto}/${fiat}...`);
    ...
    fetchExchangeRate(crypto, fiat).then(rate => {
      if (rate) {
        ...
        console.warn('Rate is null, could not update inputs.');
      }
    });
  } else {
    console.log('Auto rate not checked.');
  }
}
...
window.addEventListener('load', () => {
  console.log('Page loaded. Checking auto rate...');
  updateAutoRate();
});
